    ...
    <nav class="mb-6 text-sm">
        <ol class="list-none p-0 inline-flex">
            <li class="flex items-center">
                <a href="{{ route('admin.manuscripts.index') }}" class="text-blue-600 hover:text-blue-800">Manuscripts</a>
            ...
            <li class="text-gray-500">{{ $manuscript->title }}</li>
        </ol>
    </nav>
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="flex items-start justify-between mb-6">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center space-x-4 text-sm text-gray-500">
                            ...
                            <span>•</span>
                            <span>{{ $manuscript->pages ? number_format($manuscript->pages) . ' pages' : 'Pages not specified' }}</span>
                            <span>•</span>
                            <span>{{ $manuscript->language ?? 'English' }}</span>
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full {{ $manuscript->getStatusBadgeClass() }}">
                            {{ $manuscript->formatted_status }}
                        </span>
                        @if($manuscript->suggested_price)
                            <div class="text-sm text-green-600 mt-2">
                                Suggested Price: ${{ number_format($manuscript->suggested_price, 2) }}
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @if(!$manuscript->isPublished())
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <form action="{{ route('admin.manuscripts.review', $manuscript->id) }}" method="POST">
                        @csrf
                        <div class="space-y-4">
                            <!-- Review Action -->
                            <div>
                            ...
                            <div id="suggested-price-field" class="hidden">
                                ...
                                <input type="number" name="suggested_price" id="suggested_price" step="0.01" min="0"
                                       value="{{ $manuscript->suggested_price }}"
                            ...
                            <div>
                                ...
                                <textarea name="admin_notes" id="admin_notes" rows="5" required
                                          ...
                                          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('admin_notes', $manuscript->admin_notes) }}</textarea>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <div class="text-gray-900">{{ $manuscript->author->name }}</div>
                    </div>
                    <div>
                        <div class="text-sm font-medium text-gray-500">Email</div>
                        <div class="text-gray-900">
                            <a href="mailto:{{ $manuscript->author->email }}" class="text-blue-600 hover:text-blue-800">
                                {{ $manuscript->author->email }}
                            </a>
                        </div>
                    </div>
                    @if($manuscript->author->phone)
                        <div>
                            <div class="text-sm font-medium text-gray-500">Phone</div>
                            <div class="text-gray-900">{{ $manuscript->author->phone }}</div>
                        </div>
                    @endif
                    @if($manuscript->author->website)
                        <div>
                            <div class="text-sm font-medium text-gray-500">Website</div>
                            <div class="text-gray-900">
                                <a href="{{ $manuscript->author->website }}" target="_blank" class="text-blue-600 hover:text-blue-800">
                                    {{ $manuscript->author->website }}
                                </a>
                            </div>
                        </div>
                    @endif
                    @if($manuscript->author->bio)
                        <div>
                            <div class="text-sm font-medium text-gray-500">Bio</div>
                            <div class="text-gray-900 text-sm">{{ Str::limit($manuscript->author->bio, 200) }}</div>
                        </div>
                    @endif
                </div>

                <div class="mt-4 pt-4 border-t">
                    <a href="{{ route('admin.authors.show', $manuscript->author->id) }}"
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm text-gray-900">{{ $manuscript->created_at->format('M j, Y g:i A') }}</span>
                    </div>
                    @if($manuscript->reviewed_at)
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-500">Last Reviewed</span>
                            <span class="text-sm text-gray-900">{{ $manuscript->reviewed_at->format('M j, Y g:i A') }}</span>
                        </div>
                    @endif
                    @if($manuscript->reviewer)
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-500">Reviewed By</span>
                            <span class="text-sm text-gray-900">{{ $manuscript->reviewer->name ?? 'Admin' }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-sm font-medium text-gray-500">Language</span>
                        <span class="text-sm text-gray-900">{{ $manuscript->language ?? 'English' }}</span>
                    </div>
                    @if($manuscript->pages)
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-500">Pages</span>
                            <span class="text-sm text-gray-900">{{ number_format($manuscript->pages) }}</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('admin.manuscripts.index', ['status' => $manuscript->status]) }}"
                       class="block w-full px-4 py-2 text-center text-sm font-medium text-gray-600 bg-gray-50 border border-gray-200 rounded-md hover:bg-gray-100">
                        View Similar Status
                    </a>
                </div>
