<x-author-layout>
    <x-slot:heading>
        Edit Manuscript: {{ $manuscript->title }}
    </x-slot:heading>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-6 text-sm">
            <ol class="list-none p-0 inline-flex">
                <li class="flex items-center">
                    <a href="{{ route('author.manuscripts') }}" class="text-blue-600 hover:text-blue-800">My Manuscripts</a>
                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-amber-700">
                            <p class="whitespace-pre-wrap">{{ $manuscript->admin_notes }}</p>
                        </div>
        ...
        <div class="bg-white rounded-lg shadow p-8">
            <div class="mb-6">
                ...
                <div class="mt-3 flex items-center text-sm">
                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $manuscript->getStatusBadgeClass() }}">
                        Current Status: {{ $manuscript->formatted_status }}
                    </span>
                    <span class="ml-4 text-gray-500">
                        Originally submitted {{ $manuscript->created_at->format('M j, Y') }}
                    </span>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('author.manuscripts.update', $manuscript->id) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($genres as $genre)
                                <option value="{{ $genre }}" {{ old('genre', $manuscript->genre) === $genre ? 'selected' : '' }}>
                                    {{ $genre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                        >
                            <option value="English" {{ old('language', $manuscript->language) === 'English' ? 'selected' : '' }}>English</option>
                            <option value="Spanish" {{ old('language', $manuscript->language) === 'Spanish' ? 'selected' : '' }}>Spanish</option>
                            <option value="French" {{ old('language', $manuscript->language) === 'French' ? 'selected' : '' }}>French</option>
                            <option value="German" {{ old('language', $manuscript->language) === 'German' ? 'selected' : '' }}>German</option>
                            <option value="Italian" {{ old('language', $manuscript->language) === 'Italian' ? 'selected' : '' }}>Italian</option>
                            <option value="Portuguese" {{ old('language', $manuscript->language) === 'Portuguese' ? 'selected' : '' }}>Portuguese</option>
                            <option value="Other" {{ old('language', $manuscript->language) === 'Other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            max="10000"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('pages', $manuscript->pages) }}"
                            placeholder="e.g., 250"
                        >
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            step="0.01"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('suggested_price', $manuscript->suggested_price) }}"
                ...
                <div>
                    ...
                    >{{ old('description', $manuscript->description) }}</textarea>
